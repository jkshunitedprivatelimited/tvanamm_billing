...
import { businessDate, businessDateEquals, resolveAdminRouting, type MembershipRow } from './membership.js';
...
describe('resolveAdminRouting', () => {
  it('rejects when there is no usable membership', () => {
    expect(resolveAdminRouting([m('central_admin', 'suspended')])).toEqual({ outcome: 'no_admin_access' });
  });
  ...
  it('sends the accountant to /reports', () => {
    expect(resolveAdminRouting([m('accountant')])).toMatchObject({ outcome: 'single_workspace', redirectTo: '/reports' });
  });
  it('shows the selector for more than one', () => {
    expect(resolveAdminRouting([m('central_admin'), m('accountant')]).outcome).toBe('select_workspace');
  });
});
...
describe('businessDate', () => {
  ...
  it('compares by value', () => {
    expect(businessDateEquals({ year: 2026, month: 9, day: 4 }, { year: 2026, month: 9, day: 4 })).toBe(true);
  });
});
