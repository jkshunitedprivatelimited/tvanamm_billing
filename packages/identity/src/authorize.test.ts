...
import { authorize, scopeContains } from './authorize';
...
describe('scopeContains', () => {
  ...
  it('outlet scope matches only that outlet', () => {
    ...
    expect(scopeContains(m, { organizationId: ORG, franchiseId: FR_A, outletId: OUT_A2 })).toBe(
      false,
    );
  });
});
...
describe('authorize - capability matrix', () => {
  const cases: { role: ActorContext['role']; allow: Capability[]; deny: Capability[] }[] = [
    {
      role: 'central_admin',
      allow: ['billing.outlet.create', 'billing.outlet.lifecycle', 'catalog.menu.publish'],
      deny: ['billing.sale.create', 'billing.refund.full', 'billing.discount.apply'],
    },
    {
      role: 'accountant',
      allow: ['billing.report.global', 'billing.accounting.adjust', 'identity.audit.read'],
      deny: ['billing.sale.create', 'identity.employee.manage', 'billing.outlet.create'],
    },
    {
      role: 'franchise_owner',
      allow: ['identity.employee.manage', 'billing.refund.partial', 'billing.outlet.manage'],
      deny: ['billing.outlet.create', 'billing.outlet.lifecycle', 'catalog.menu.manage.master'],
    },
    {
      role: 'store_employee',
      allow: ['billing.sale.create', 'billing.cash_session.close', 'billing.shift.open'],
      deny: ['billing.report.global', 'identity.employee.manage', 'billing.outlet.manage'],
    },
  ];
  for (const { role, allow, deny } of cases) {
    ...
    for (const cap of allow) {
      it(`${role} allowed ${cap}`, () => {
        expect(authorize(actor, cap, requested).allowed).toBe(true);
      });
    }
    for (const cap of deny) {
      it(`${role} denied ${cap}`, () => {
        expect(authorize(actor, cap, requested).allowed).toBe(false);
      });
    }
  }
});
...
describe('authorize - gate clauses', () => {
  ...
  it('denies an inactive admin account', () => {
    ...
    expect(authorize(actor, 'identity.employee.manage', scope)).toEqual({
      allowed: false,
  ...
  it('denies out-of-scope', () => {
    expect(
      authorize(admin('franchise_owner', scope), 'identity.employee.manage', {
        organizationId: ORG,
        franchiseId: FR_B,
      }),
    ).toEqual({
  ...
  it('requires fresh auth when the policy demands it', () => {
    ...
    expect(
      authorize(base, 'identity.employee.manage', scope, { requireFreshAuthWithinSeconds: 300 }),
    ).toEqual({
      allowed: false,
      reason: 'fresh_auth_required',
    });
    expect(
      authorize({ ...base, secondsSinceAuth: 60 }, 'identity.employee.manage', scope, {
        requireFreshAuthWithinSeconds: 300,
      }).allowed,
    ).toBe(true);
  });
});
