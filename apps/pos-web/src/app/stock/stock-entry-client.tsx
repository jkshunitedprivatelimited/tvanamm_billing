...
export function StockEntryClient({ items, entries }: { items: Item[]; entries: Entry[] }) {
  ...
  return (
    <>
      <nav className="row wrap" style={{ gap: 10, marginBottom: 20 }}>
        ...
        <a href="/expenses">Other expense</a>
      </nav>
      {pending.length ? (
      ...
      >
        ...
        <p>
          {kind === 'purchase'
            ? 'For example: milk received this morning. Enter the total paid, not the price per litre.'
            : kind === 'wastage'
        ...
        >
          <label>
            ...
            >
              ...
              {items
                .filter((i) => kind !== 'purchase' || i.supplyRule !== 'jksh_required')
                .map((i) => (
          ...
          <label>
            ...
            <input
              ...
              value={reason}
              placeholder={
                kind === 'purchase' ? 'Morning milk from local dairy' : 'Describe what happened'
              }
      ...
      <section className="card">
        ...
        {!entries.some((e) => e.result) ? (
          <p className="muted">Your saved purchases, wastage and counts will appear here.</p>
        ) : null}
        {entries
          .filter((e) => e.result)
          .map((e) => (
