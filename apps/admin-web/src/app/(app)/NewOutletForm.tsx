...
export function NewOutletForm() {
  ...
  async function submit(e: React.SyntheticEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(((await res.json()) as { message?: string }).message ?? 'Could not create the outlet.');
        return;
      }
      setOpen(false);
      setForm({ displayName: '', ownershipType: 'franchise_owned', franchiseId: '', city: '', state: '', gstin: '' });
      router.refresh();
    } finally {
      setBusy(false);
    }
  ...
  return (
    <form className="card" onSubmit={submit}>
      ...
      <input id="dn" value={form.displayName} onChange={(e) => set('displayName', e.target.value)} required />
      ...
      {form.ownershipType === 'franchise_owned' ? (
        <>
          <label htmlFor="fr">Franchise ID</label>
          <input id="fr" value={form.franchiseId} onChange={(e) => set('franchiseId', e.target.value)} required />
        </>
      ) : null}
      ...
      <input id="gstin" value={form.gstin} onChange={(e) => set('gstin', e.target.value.toUpperCase())} maxLength={15} />
      <button type="submit" disabled={busy || form.displayName.trim().length < 2}>
